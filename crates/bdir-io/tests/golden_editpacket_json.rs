use bdir_io::{core::Document, editpacket};
...
fn golden_edit_packet_pretty_json() {
    ...
    let packet = editpacket::from_document(&doc, None);

    let pretty = editpacket::to_pretty_json(&packet).expect("pretty json");
...
fn golden_edit_packet_minified_json() {
    ...
    let min = editpacket::to_minified_json(&packet).expect("minified json");
